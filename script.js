...
function getMetaSpeakers() {
	if (Array.isArray(meta.speakers) && meta.speakers.length) return meta.speakers;
	if (meta.speaker && typeof meta.speaker === "object") return [meta.speaker];
	return [];
}
...
function renderSlide(s, idx) {
	...
	var footer = s.footer ? '<div class="footer"><span>' + s.footer + "</span><span>" + "</span></div>" : "";
	// var hint = s.hint ? '<div class="hint">&#9201; ' + escapeHtml(s.hint) + '</div>' : '';
	var kicker = s.kicker ? '<div class="kicker ' + (s.kickerClass || "") + '">' + s.kicker + "</div>" : "";
	return kicker + renderer(s) + footer;
}

var renderers = {
	title: function (s) {
		var speakers = getMetaSpeakers();
		var label = speakers.length > 1 ? "Speakers" : "Speaker";
		var blocks = speakers
			.map(function (spk) {
				return (
		...
		return (
			...
			s.title +
			"</h1>" +
			'<p class="big" style="margin-top:12px;max-width:1100px">' +
			s.subtitle +
			"</p>" +
	...
	split: function (s) {
		...
		return "<h2>" + s.heading + "</h2>" + '<div class="split">' + renderHalf(s.left) + renderHalf(s.right) + "</div>" + bigNote;
	},
	...
	chat: function (s) {
		...
		return "<h2>" + s.heading + "</h2>" + lead + '<div class="chat">' + messages + "</div>" + bigNote;
	},
	...
	diagram: function (s) {
		...
		return "<h2>" + s.heading + "</h2>" + pull + '<div class="diagram">' + nodes + "</div>" + '<div class="row">' + cards + "</div>";
	},
	...
	architecture: function (s) {
		...
		var cards = (s.cards || [])
			...
			.join("");
		return "<h2>" + s.heading + "</h2>" + '<div class="timeline">' + steps + "</div>" + '<div class="row" style="margin-top:40px">' + cards + "</div>";
	},
	...
	tools: function (s) {
		...
		return "<h2>" + s.heading + "</h2>" + '<div class="tools-grid">' + tools + "</div>" + note;
	},
	...
	cards: function (s) {
		...
		return "<h2>" + s.heading + "</h2>" + '<div class="row">' + cards + "</div>" + bigNote;
	},
	...
	challenge: function (s) {
		...
		return "<h2>" + s.heading + "</h2>" + '<div class="challenge-list">' + steps + "</div>" + bigNote;
	},
	...
	closing: function (s) {
		var columns = (s.columns || [])
			...
			.join("");
		return '<h1 style="font-size:92px">' + s.heading + "</h1>" + '<div class="closing-grid">' + '<div class="qr-placeholder">' + s.qrText + "</div>" + columns + "</div>";
	},
